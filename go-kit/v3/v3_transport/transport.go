...
func decodeHTTPLoginRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	...
	utils.GetLogger().Debug(fmt.Sprint(ctx.Value(v3_service.ContextReqUUid)), zap.Any(" 开始解析请求数据", login))
	return login, nil
}
...
func decodeHTTPADDRequest(ctx context.Context, r *http.Request) (interface{}, error) {
	...
	utils.GetLogger().Debug(fmt.Sprint(ctx.Value(v3_service.ContextReqUUid)), zap.Any(" 开始解析请求数据", in))
	return in, nil
}
